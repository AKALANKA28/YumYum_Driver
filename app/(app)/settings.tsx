...
export default function SettingsScreen() {
  ...
  useEffect(() => {
    async function loadUserData() {
      try {
        // Try to get data from auth state first
        if (authState.driver) {
          setProfileData(authState.driver);
          setLoading(false);
          return;
        }

        // As fallback, try to get from AsyncStorage
        const driverData = await AsyncStorage.getItem("driver");
        if (driverData) {
          setProfileData(JSON.parse(driverData));
        } else {
          console.log("No driver data found in storage");
        }
      } catch (error) {
  ...
  // Get user's email
  const userEmail = profileData?.email || "No email provided";
        ...
        <SettingItem onPress={navigateToSupport}>
          <SettingItemLeft>
            ...
            <View>
              ...
              <SettingDescription>
                Get help with issues and questions
              </SettingDescription>
            </View>
          </SettingItemLeft>
          <Ionicons name="chevron-forward" size={20} color="#999" />
